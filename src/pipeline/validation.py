        ...
        with HttpFetcher(settings) as fetcher:
            for src in sources:
                ...
                if probe.status == AccessStatus.ACCESSIBLE.value:
                    # Reachable — does the parser actually extract anything?
                    ...
                    if candidates:
                        ...
                        # Accessible but nothing parsed. If we'd normally expect
                        # results, that's a parser problem; otherwise partial.
                        live = (SourceLiveStatus.PARSER_BROKEN.value
                                if (src.typical_result_count or 0) >= 3
                                else SourceLiveStatus.PARTIALLY_WORKING.value)

                if probe.status == AccessStatus.JS_REQUIRED.value:
                ...
                src.live_status = live or SourceLiveStatus.UNKNOWN.value
                src.live_status_detail = detail[:300]
                src.live_checked_at = datetime.now(timezone.utc)
                counts[src.live_status] = counts.get(src.live_status, 0) + 1
                checked += 1
